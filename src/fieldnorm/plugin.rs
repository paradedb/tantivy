...
/// The built-in [`SegmentPlugin`] that manages field norms.
...
/// The [`PluginWriter`] for the field norms plugin.
pub struct FieldNormsPluginWriter {
    /// The inner field norms writer accumulating per-document field norms.
    pub writer: FieldNormsWriter,
    serializer: Option<FieldNormsSerializer>,
}

impl FieldNormsPluginWriter {
    /// Records the field norm of a single field for a single document.
    pub fn record(&mut self, doc: DocId, field: crate::schema::Field, fieldnorm: u32) {
        self.writer.record(doc, field, fieldnorm);
    }

    /// Fills the field norms with default values up to the given `max_doc`.
