...
/// File extensions for built-in segment components that no plugin owns: the delete
/// bitset (`<uuid>.<opstamp>.del`). It is reserved so a custom plugin cannot claim it
/// and contend for the file.
const RESERVED_NON_PLUGIN_EXTENSIONS: &[&str] = &["del"];
...
impl IndexBuilder {
    ...
    /// Registers a custom [`SegmentPlugin`] on the index being built.
    ///
    /// The plugin participates in the segment write, read, and merge lifecycle alongside
    /// the built-in plugins. The set of custom extensions is recorded at index creation.
    #[must_use]
