...
fn set_centroids_round_trip_and_drive_assignment() -> crate::Result<()> {
    ...
    let decoded_centroids = (0..field_centroids.num_centroids())
        .flat_map(|centroid| {
            field_centroids
        ...
        .collect::<Vec<_>>();
    assert_eq!(decoded_centroids, centroid_values);
...
/// The centroid-index file stores the selected router's discriminant.
#[test]
fn centroid_index_writes_router_kind() -> crate::Result<()> {
    let centroids = vec![[0.0, 0.0], [6.0, 6.0]];
    let index = TestVectorIndex::builder(VectorDType::F32)
        .centroids(&centroids)
        .build()?;
    let set = open_centroid_index(&index.index)?;
    let router_bytes = set.router_slice(index.embedding_field())?.read_bytes()?;
    assert_eq!(router_bytes[0], RouterKind::Rng as u8);
    Ok(())
}
...
mod centroid_index_lifecycle_tests {
    ...
    use crate::vector::{Metric, RouterKind, VectorOptions};
    ...
    #[test]
    fn centroid_index_requires_explicit_router() {
        let err = Index::builder()
            .schema(vector_schema())
            .centroid_producer(Arc::new(SingleCellCentroidProducer { dim: 2 }))
            .create_in_ram()
            .unwrap_err();
        assert!(
            err.to_string().contains("explicit IVF router"),
            "unexpected: {err}"
        );
    }

    #[test]
    fn stacked_router_round_trips_through_global_centroid_index() -> crate::Result<()> {
        let directory = RamDirectory::create();
        let centroids = (0..32)
            .map(|i| [i as f32, (i % 5) as f32])
            .collect::<Vec<_>>();
        let index = Index::builder()
            .schema(vector_schema())
        ...
        let embedding = index.schema().get_field("embedding")?;
        let mut writer = index.writer_with_num_threads(1, 15_000_000)?;
        let mut doc = TantivyDocument::new();
        doc.add_vector(embedding, &[1.0_f32, 1.0]);
        writer.add_document(doc)?;
        writer.commit()?;
        drop(writer);
        drop(index);

        let mut mismatched = Index::open(directory.clone())?;
        mismatched.set_ivf_router(RouterKind::Rng)?;
        let err = match mismatched.cached_centroid_index() {
            Ok(_) => panic!("mismatched router opened the centroid index"),
            Err(err) => err,
        };
        assert!(
            err.to_string()
                .contains("configured router rng does not match persisted router stacked"),
            "unexpected: {err}"
        );

        let mut reopened = Index::open(directory)?;
        reopened.set_ivf_router(RouterKind::Stacked)?;
        let cached = reopened.cached_centroid_index()?;
        let router = cached
            .field_router(embedding)
            .expect("field router")
            .router();
        assert_eq!(router.kind(), RouterKind::Stacked);
        Ok(())
    }
...
mod bounds_storage_tests {
    ...
    use crate::vector::{
        residual_norm, BoundKind, Metric, RouterKind, VectorDType, VectorOptions, VEC_EXT,
    };
    use crate::{Index, IndexWriter, TantivyDocument};
