...
mod tests {
    ...
    #[test]
    fn global_stacked_routing_uses_one_budget_and_recall_target() -> crate::Result<()> {
        ...
        match stats.routing {
            Some(RouterMetrics::Stacked {
                candidate_count,
                recall_target,
                ..
            }) => {
                assert_eq!(candidate_count, 2);
                assert_eq!(recall_target, 0.7);
            }
            routing => panic!("expected stacked routing, got {routing:?}"),
        }
        ...
        assert!(matches!(
            stats.routing,
            Some(RouterMetrics::Stacked {
                candidate_count: DEFAULT_NUM_CENTROIDS,
                recall_target: 1.0,
                ..
            })
        ));
