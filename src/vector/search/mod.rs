...
//!
//! Flat (unclustered) segments never reach this driver: they have no
//! cross-segment state, so the collector routes them through the
//! ordinary per-segment path
//! ([`VectorSegmentCollector`](collector::VectorSegmentCollector)) and
//! merges the fruits. Segments in the filter without clusters are
//! skipped here.
...
use self::stats::{ProbeStats, ProbeTermination};
use self::work::{open_share, WorkUnits};
...
use super::reader::VectorIndexReader;
...
pub(crate) fn global_top_n_by<T, S>(
    ...
    tie_break: &S,
    segment_filter: Option<&[SegmentOrdinal]>,
    ...
    S: SortKeyComputer,
{
    ...
    for (ord, reader) in searcher.segment_readers().iter().enumerate() {
        // A filtered run behaves exactly like a snapshot holding only the
        // listed segments — budget capacity included.
        if let Some(filter) = segment_filter {
            if !filter.contains(&(ord as SegmentOrdinal)) {
                continue;
            }
        }
