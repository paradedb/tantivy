...
use super::ivf::{AdaptiveProbeParams, Candidate, SegmentClusters};
    ...
    S: SortKeyComputer,
{
    ...
    if !segments.is_empty() {
        ...
        // the router scores unit-norm rows with a raw dot kernel, so the
        // QUERY is normalized here, once —
        ...
        let routing = super::RoutingParams {
            k: adaptive.router_k(work_budget.get(), num_centroids),
            recall: adaptive.router_recall_target,
        };
        let mut ranked = router.rank_clusters(&mut routing_workspace, &query_f32, routing);
