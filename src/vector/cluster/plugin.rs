...
fn serialize_windowed_field(
    ...
    // Pre-serialize each window's bytes so we know its length for the directory.
    ...
    // Header preamble was: 4 (num_windows) + 4 (window_size) + FIELD_META_SIZE
    // Then directory: per window 8 (off) + 8 (len) + 4 (doc_offset) + 4 (num_docs) = 24 bytes
...
fn serialize_windowed_empty(
    ...
    // Pre-serialize each empty window
...
impl WindowReader {
    /// Open a window reader from raw bytes.
    /// `data` is the bytes for this window only.
    /// `base_offset` is the offset within `data` to start reading at.
    /// `file_base_offset` is the absolute offset in `file_slice` where `data` begins;
    /// used when slicing out `postings_data` / `batch_data` ranges.
    fn open(
...
pub struct ClusterFieldReader {
    ...
    /// Lazily opened windows. Inner Option distinguishes "not yet opened" (None)
    /// from "opened" (Some). Wrapped in OnceLock for thread-safe lazy init.
...
impl ClusterFieldReader {
    fn open(file_slice: FileSlice, metric: Metric) -> crate::Result<Self> {
        // Only read the header + directory upfront, not the per-window data.
        // Read enough bytes for: num_windows (4) + window_size (4) + field_meta + a bit more.
        // We do an initial small read for the preamble, then a second read for the directory.
        let preamble_size = 4 + 4 + FIELD_META_SIZE;
        ...
        // Now read the directory: num_windows × 24 bytes
        ...
        let windows = (0..num_windows).map(|_| once_cell::sync::OnceCell::new()).collect();
    ...
    pub fn is_clustered(&self) -> bool {
        // We can only tell whether ANY window is clustered by opening at least one.
        // Use the first window as a proxy: if it's empty (num_clusters = 0), it's not clustered.
        // For safety / correctness, check if we have any windows at all.
        !self.directory.is_empty()
    }

    pub fn num_clusters(&self) -> usize {
        // Sum requires loading every window; only used for diagnostics.
    ...
    pub fn dim_bytes(&self) -> usize {
        // dim_bytes was historically derived from any opened window's dims field.
        // Since SortByVectorDistance reads padded_dims directly from VectorFieldMeta,
        // we no longer need to track this separately.
