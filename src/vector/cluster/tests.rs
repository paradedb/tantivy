...
fn collect_postings_doc_ids(
    ...
    // Doc IDs are stored at the start of each cluster's batch_data.
    match reader.cluster_batch_raw(cluster_id) {
        Ok(Some((doc_ids, _, _))) => doc_ids,
        _ => Vec::new(),
    }
}
...
// Adaptive probe-cutoff behavior is exercised end-to-end in
// SortByVectorDistance integration tests; the standalone probe_clusters
// API was removed.
