...
mod rng_tests {
    ...
    fn lazy_adjacency_reads_only_requested_groups_and_preserves_mutation() -> io::Result<()> {
        ...
        assert_eq!(
            file.requests.lock().unwrap().as_slice(),
            std::slice::from_ref(&(0..4))
        );
        assert!(lazy.neighbors.is_empty());
        assert_eq!(lazy.lazy_neighbors.as_ref().unwrap().groups.len(), 3);
        ...
        assert_eq!(
            lazy.lazy_neighbors
                ...
                .groups
                .iter()
                .map(|group| group.get().unwrap().len())
                .sum::<usize>(),
            17 * 3
        );
    ...
    fn lazy_adjacency_group_boundaries() -> io::Result<()> {
        for count in [7usize, 8, 9, 19] {
            ...
            let mut expected: Vec<_> = std::iter::once(0..4).collect();
        ...
        for metric in [Metric::L2, Metric::Dot, Metric::Cosine] {
            ...
            for prefix in [1, 16, 64, 129, 257, usize::MAX] {
                for copying in [false, true] {
                    ...
                    let mut ei = eager.search_iter(&mut ew, &query, &seeds);
                    let mut li = lazy.search_iter(&mut lw, &query, &seeds);
    ...
    #[test]
    fn segment_router_reads_only_demanded_adjacency() -> crate::Result<()> {
        use crate::schema::VectorOptions;
        use crate::vector::header::VectorFileVersion;
        use crate::vector::router::{RouterKind, RouterWorkspace};

        let built = line_index(4096);
        let mut bytes = vec![RouterKind::Rng as u8];
        built.serialize(&mut bytes)?;
        let file = std::sync::Arc::new(CountedAdjacency {
            bytes: common::OwnedBytes::new(bytes.clone()),
            requests: Default::default(),
        ...
        assert_eq!(*file.requests.lock().unwrap(), [0..1, 1..5]);
        let eager = RelativeNeighborhoodGraph::open(
            &bytes[1..],
            built.graph.arena().as_slice(),
            1,
            Metric::L2,
            NeighborhoodGraphConfig::default(),
        )?;
        let query = [54.5];
        let seeds: Vec<_> = (0..4096).step_by(512).collect();
        let mut eager_workspace = Workspace::new();
        let expected: Vec<_> = eager
            .search_iter(&mut eager_workspace, &query, &seeds)
            .take(32)
            .map(|candidate| (candidate.node, candidate.sim.score().to_bits()))
            .collect();
        let mut workspace = RouterWorkspace::default();
        for _ in 0..2 {
            let actual: Vec<_> = router
                .rank(&mut workspace, &query, Metric::L2)
                .take(32)
                .map(|candidate| (candidate.node, candidate.sim.score().to_bits()))
                .collect();
            assert_eq!(actual, expected);
        ...
        let requests = file.requests.lock().unwrap();
        let mut unique = std::collections::HashSet::new();
        for range in &requests[2..] {
            assert!(unique.insert(range.start));
            assert!(range.len() <= ADJACENCY_GROUP_NODES * 4 * built.config.max_edges);
        }
        let requested_bytes: usize = requests.iter().map(|range| range.len()).sum();
        assert!(requested_bytes < bytes.len());
        eprintln!(
            "4096-node segment router, 32 results: {requested_bytes}/{} adjacency bytes, {} \
             groups, repeated route uses cached groups",
            bytes.len(),
            unique.len()
        );
        Ok(())
