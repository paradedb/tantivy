...
impl IvfVecReader {
    pub(crate) fn open(ctx: &PluginReaderContext, meta_slice: FileSlice) -> crate::Result<Self> {
...
impl VectorColumnReader for IvfVecReader {
    ...
    fn open_column(&self, field: Field) -> crate::Result<IvfVectorColumn> {
        ...
        let assignments_segment = self.segment.clone();
        let assignments_slice = DeferredFileSlice::new(move || {
            let assignments_file = assignments_segment
