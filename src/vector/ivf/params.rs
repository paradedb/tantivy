...
/// `n_bar = N / C`, native docs over clusters. One constant; no
...
impl WorkModel {
    /// Compute `n_bar` for `field` across `searcher`'s IVF segments.
...
///
/// The budget is measured in WORK UNITS, not raw clusters: 1 unit is one
/// average cluster of work, charged event-wise as probing proceeds. The
/// pricing lives in `ChargeModel`, not here - this struct resolves what a
/// segment may spend, never what an event costs.
#[derive(Clone, Debug)]
pub struct ProbeBudget {
    /// The fraction of the index's probe WORK a query may spend, resolved
    /// per segment as `f * units_seg` - see
    /// [`Self::resolved_work_budget`]. A fraction rather than an absolute
    /// count because segment sizes vary: an absolute cap over-probes small
    /// segments (clamping to a full scan) and under-probes large ones.
    /// Uneven clusters charge proportionally, so `f` keeps its meaning
    /// whatever the size skew. This is the single user-facing tuning knob
    /// for IVF probing. SPANN Fig. 2: 99% of SIFT1M queries reach perfect
    /// recall@1 within 114 postings, ~1% of a 1%-centroid-ratio index's
    /// clusters. Default 0.01, PROVISIONAL.
...
impl ProbeBudget {
    ...
    /// `units_seg = C_seg*x + (1 - x)*N_seg/n_bar` (n_bar global when
    ...
    ///
    /// Returns `(budget, n_bar, x)` - the charge model is built from all
    /// three. A non-positive fraction is a configuration error, not "no
    /// probing".
    pub(crate) fn resolved_work_budget(
        ...
        let x = crate::vector::backend::open_share(n_bar);
        let units_seg = c_seg as f64 * x + (1.0 - x) * n_seg as f64 / n_bar.max(f64::MIN_POSITIVE);
...
#[cfg(test)]
mod tests {
    use super::{ProbeBudget, WorkModel};

    /// Capacity is the cluster count, exactly, whatever the granularity:
    /// `units_seg = C*x + (1 - x)*N/n_bar == C` for every x the open
    ...
    fn unprimed_capacity_is_the_cluster_count() -> crate::Result<()> {
        ...
        for (c_seg, n_seg) in [(1000usize, 20_000usize), (9, 20), (2, 2), (64, 6400)] {
            let (budget, _n_bar, _x) = all.resolved_work_budget(c_seg, n_seg)?;
            assert!(
    ...
    fn primed_model_allocates_by_work() -> crate::Result<()> {
        ...
        assert!(big > small, "the fuller segment gets more: {big} vs {small}");
        assert!(
