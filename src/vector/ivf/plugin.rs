...
mod tests {
    ...
    fn level_zero_matches_unquantized_ivf() -> crate::Result<()> {
        ...
        let Some(crate::vector::RouterMetrics::Rng(level_zero_routing)) = level_zero.routing else {
            panic!("expected RNG routing: {level_zero:?}");
        };
        let Some(crate::vector::RouterMetrics::Rng(baseline_routing)) = baseline.routing else {
            panic!("expected RNG routing: {baseline:?}");
        };
        assert!(level_zero_routing.visited_count > 0, "{level_zero:?}");
        ...
        assert_eq!(
            level_zero_routing.visited_count,
            baseline_routing.visited_count
        );
        assert_eq!(
