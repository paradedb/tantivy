...
//! the target segment's doc count meets the clustering threshold, and
//! `VectorPlugin` isn't registered yet, so existing tests are
//! unaffected.
...
///   1. Read source vectors. For each source, prefer the flat column if present; otherwise
///      reconstruct from the source's own IVF data (vectors live inside the per-cluster blob).
///   2. Train centroids — e.g., k-means over a sample of the assembled vectors. K, sample size,
///      iteration cap, and seed live on a forthcoming `IvfVecConfig`.
