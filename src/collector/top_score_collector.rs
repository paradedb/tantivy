...
impl TopDocs {
    ...
    /// [`VectorOptions`](crate::vector::VectorOptions) at query time —
    ...
    pub fn order_by_similarity<T>(
        ...
        query: Vec<T>,
    ) -> crate::vector::TopDocsByVectorSimilarity<T>
    where
        T: crate::vector::VectorElement,
    {
        crate::vector::TopDocsByVectorSimilarity::new(field, query, self.limit)
    ...
    C: Comparator<TSortKey>,
{
    ...
    /// Out-of-order callers (e.g., index formats that visit docs
    /// cluster-by-cluster rather than in doc-id order) can't make that
    /// guarantee, so this path relaxes the skip to strictly less than
    /// the threshold. Equal-sort-key candidates get appended and are
    /// resolved by the full sort-key + doc-id compare in
    /// [`truncate_top_n`](Self::truncate_top_n) / [`into_sorted_vec`](Self::into_sorted_vec).
    ///
    /// Same asymptotic cost; slightly worse constant on tied scores.
    #[inline]
    pub fn push_unordered(&mut self, sort_key: TSortKey, doc: D) {
        if let Some(threshold) = &self.threshold {
            if self.comparator.compare(&sort_key, threshold) == Ordering::Less {
                return;
            }
        }
        self.append_doc(doc, sort_key);
