    ...
    C: Comparator<TSortKey>,
{
    ...
    /// The exact current n-th best sort key, or `None` while the buffer
    /// holds fewer than `top_n` entries — with no externally-set threshold
    /// that is exactly "fewer than `top_n` entries ever collected": nothing
    /// is dropped before the first truncation, and truncation never shrinks
    /// the buffer below `top_n`. Also `None` for `top_n == 0`, where "n-th
    /// best" is undefined.
    ///
    /// * Returns (`Option<TSortKey>`): the n-th best key under this computer's comparator, cloned
    ///   out of the buffer.
    ///
    /// Side effect: a buffer holding more than `top_n` entries is
    /// force-truncated to exactly `top_n` through the same machinery the
    /// capacity overflow uses, which also updates `threshold` — so
    /// subsequent [`push`](Self::push) / [`push_unordered`](Self::push_unordered)
    /// early-drops kick in sooner than they would have without this call.
    /// Each call is O(buffer); intended for coarse boundaries (e.g. once
    /// per probed IVF cluster), not per push.
...
mod tests {
    ...
    fn test_kth_best_tracks_improvement() {
        // As better entries displace the tail, the n-th best rises — and
        // it is exact at every boundary, not lagging like `threshold`.
        let mut computer: TopNComputer<u32, u32, NaturalComparator> =
    ...
    fn test_kth_best_after_natural_truncation() {
        // Overflow the 2*top_n buffer so `truncate_top_n` has already
        // fired naturally; `kth_best` must still be the exact n-th best of
        // everything pushed.
        let top_n = 4usize;
        let mut computer: TopNComputer<u32, u32, NaturalComparator> =
    ...
    fn test_kth_best_with_push_unordered_natural_comparator() {
        // The vector-backend shape: f32 scores, NaturalComparator (higher
        // is better), out-of-order pushes. Interleave queries with pushes
        // and confirm both the boundary values and the final top-K.
        ...
        // Below the n-th best: dropped by the truncation threshold, and
        // the n-th best is unchanged.
