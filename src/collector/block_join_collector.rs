...
mod tests {
    ...
    use crate::schema::{Field, IndexRecordOption, Schema, SchemaBuilder, INDEXED, STORED, TEXT};
    ...
    fn build_sample_index() -> (Index, Field, Field) {
        ...
        let index = Index::create_in_ram(schema.clone());
        {
            ...
            writer
                .add_document(doc!( parent_f=>true,  title_f=>"ParentA" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>false, title_f=>"child a1" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>false, title_f=>"child a2" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>true,  title_f=>"ParentB" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>false, title_f=>"child b1" ))
                .unwrap();
    ...
    fn test_block_join_no_parents() -> Result<()> {
...

        {
            ...
            writer
                .add_document(doc!( parent_f=>false, title_f=>"foo" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>false, title_f=>"bar" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>false, title_f=>"baz" ))
                .unwrap();
            writer.commit()?;
        }
    ...
    fn test_block_join_no_children() -> Result<()> {
...

        {
            let mut writer = index.writer_for_tests()?;
            writer
                .add_document(doc!( parent_f=>true, title_f=>"Alpha" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>true, title_f=>"Beta" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>true, title_f=>"Gamma" ))
                .unwrap();
            writer.commit()?;
        }
        ...
        for (_parent_doc, kids, scores) in &fruit {
    ...
    fn test_block_join_basic() -> Result<()> {
        let (index, _title_f, parent_f) = build_sample_index();
        let reader = index.reader()?;
        let searcher = reader.searcher();
    ...
    fn test_block_join_some_children_unmatched() -> Result<()> {
...

        {
            let mut writer = index.writer_for_tests()?;
            writer
                .add_document(doc!( parent_f=>true,  title_f=>"ParentX interesting" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>false, title_f=>"interesting child" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>false, title_f=>"boring child" ))
                .unwrap();
            writer.commit()?;
        }
    ...
    fn test_block_join_multiple_segments() -> Result<()> {
...

        {
            let mut writer = index.writer_for_tests()?;
            writer
                .add_document(doc!( parent_f=>true,  title_f=>"S0P0" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>false, title_f=>"s0 child1" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>false, title_f=>"s0 child2" ))
                .unwrap();
            writer.commit()?;
        }
        {
            let mut writer = index.writer_for_tests()?;
            writer
                .add_document(doc!( parent_f=>true,  title_f=>"S1P0" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>false, title_f=>"s1 child1" ))
                .unwrap();
            writer
                .add_document(doc!( parent_f=>false, title_f=>"s1 child2" ))
                .unwrap();
            writer.commit()?;
        }
