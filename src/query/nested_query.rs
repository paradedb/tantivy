...
mod explode {
    ...
    pub fn explode_document(
        ...
        // If it's top-level and "nested", set `_is_parent_<path>` = true.
        // For path=["vehicle"], that means `_is_parent_vehicle`
        if is_top_level && opts.object_mapping_type == ObjectMappingType::Nested {
