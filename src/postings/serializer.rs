...
impl PostingsSerializer {
                    ...
                    blockwand_params = fieldnorms
                        ...
                        .inspect(|&(norm, tf)| {
                            ...
                            if cfg!(feature = "subblock-pruning") {
                                subblocks[posting_idx / super::subblock::SUBBLOCK_SIZE]
                                    .record(norm, tf);
                                posting_idx += 1;
                            }
                            if cfg!(any(
    ...
    pub fn close_term(
        ...
        if !self.block.is_empty() {
            if cfg!(any(feature = "subblock-pruning", feature = "posting-norms"))
                && self.term_has_freq
            {
                if let Some(norms) = &self.fieldnorm_reader {
                    for (docs, freqs) in self
                        .block
                        .doc_ids()
                        .chunks(super::subblock::SUBBLOCK_SIZE)
                        .zip(
                            self.block
                                .term_freqs()
                                .chunks(super::subblock::SUBBLOCK_SIZE),
                        )
                    {
                        ...
                        for (&doc, &tf) in docs.iter().zip(freqs) {
                            let norm = norms.fieldnorm_id(doc);
        ...
        if !self.subblock_summaries.is_empty() {
            assert_eq!(
                self.subblock_summaries.len(),
                (doc_freq as usize).div_ceil(super::subblock::SUBBLOCK_SIZE)
            );
