...
pub trait PluginWriter: Send + Any {
    ...
    fn serialize(&mut self, segment: &Segment) -> crate::Result<()>;
    ...
    /// Returns a `&dyn Any` view of this writer for downcasting to the concrete type.
    fn as_any(&self) -> &dyn Any;
    /// Returns a `&mut dyn Any` view of this writer for downcasting to the concrete type.
...
pub struct PluginMergeContext<'a> {
    /// The source segment readers being merged.
    pub readers: &'a [SegmentReader],
    /// Mapping from the merged (new) doc ids to the old doc addresses.
    pub doc_id_mapping: &'a SegmentDocIdMapping,
    /// The segment the merged data is written to.
    pub target_segment: &'a Segment,
    /// The schema shared by all segments being merged.
    pub schema: &'a Schema,
    /// The index settings for the merge.
    pub settings: &'a IndexSettings,
    /// Whether the document store should be ignored for this merge.
    pub ignore_store: bool,
}
...
mod tests {
    ...
    fn test_reserved_extension_plugins_fail_closed() -> crate::Result<()> {
        ...
        // The delete bitset (`del`) is not owned by any plugin, so a custom plugin
        // claiming it would contend for the reserved file.
        for reserved in ["del"] {
            let mut schema_builder = Schema::builder();
            schema_builder.add_text_field("text", TEXT | STORED);
            let index = Index::builder()
