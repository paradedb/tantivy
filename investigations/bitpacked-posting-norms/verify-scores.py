...
matrix = json.loads((root / (sys.argv[1] if len(sys.argv) > 1 else "packed-comparison-results.json")).read_text())
results = {}
with psycopg.connect("postgresql://mingying@127.0.0.1:29950/hn_benchmark?sslmode=disable", autocommit=True) as c:
    ...
    for name, measured in matrix.items():
        ...
        (root / "packed-correctness.json").write_text(json.dumps(results, indent=2))
