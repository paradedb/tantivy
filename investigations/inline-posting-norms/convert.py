...
with psycopg.connect(url,autocommit=True) as c:
    ...
    originals=[s['old'] for s in json.loads((root.parent/'norm-descriptors/conversion.json').read_text())]
    c.execute("SET lock_timeout='10s'")
    c.execute("SELECT diagnostic_select_norm_components('hn_items_idx',%s)",(Jsonb(originals),))
    before=c.execute("SELECT diagnostic_posting_norm_storage('hn_items_idx')").fetchone()[0]
    (root/'before-storage.json').write_text(json.dumps(before,indent=2))
    changes=[]
    for s in before:
        start=time.monotonic()
        change=c.execute("SELECT diagnostic_inline_posting_norms('hn_items_idx',%s)",(s['segment'],)).fetchone()[0]
        change['seconds']=time.monotonic()-start
        changes.append(change)
        (root/'conversion.json').write_text(json.dumps(changes,indent=2))
        print(s['segment'],round(change['seconds'],2),'seconds;',change['new']['postings']['total_bytes']-change['old']['postings']['total_bytes'],'additional postings bytes',flush=True)
        c.execute('CHECKPOINT')
    after=c.execute("SELECT diagnostic_posting_norm_storage('hn_items_idx')").fetchone()[0]
    (root/'after-storage.json').write_text(json.dumps(after,indent=2))
