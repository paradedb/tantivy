...
with psycopg.connect("postgresql://mingying@127.0.0.1:29950/hn_benchmark?sslmode=disable", autocommit=True) as c:
    ...
    for name, measured in matrix.items():
        ...
        for label in ["raw", "packed"]:
            ...
            c.execute("SELECT public.diagnostic_posting_norms(%s)", (label == "packed",))
            ...
            by_id = dict(rows)
            top = measured["rows"][{"raw": "inline", "packed": "inline"}[label]]
        ...
        (root / "inline-correctness.json").write_text(json.dumps(results, indent=2))
