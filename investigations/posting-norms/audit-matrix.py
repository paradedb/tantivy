...
tag = sys.argv[1] if len(sys.argv) > 1 else "audit-before"
queries = {}
...
for name, query in queries.items():
    ...
    with psycopg.connect("postgresql://mingying@127.0.0.1:29950/hn_benchmark?sslmode=disable", autocommit=True) as c:
        ...
        assert all(s["posting_norm_bytes"] for s in out["storage"])
        out["index_size_bytes"] = c.execute("SELECT pg_relation_size('hn_items_idx')").fetchone()[0]
        for iteration in range(25):
            for label in (["global", "local"] if iteration % 2 == 0 else ["local", "global"]):
                c.execute("SELECT public.diagnostic_posting_norms(%s)", (label == "local",))
        ...
        for label in ["global", "local"]:
            ...
            for run in out["runs"]:
                ...
                values.append({
                    ...
                    "posting_norm_hits": io["components"].get("pnorm", {}).get("hits", 0),
                    "fieldnorm_reads": io["components"].get("fieldnorm", {}).get("reads", 0),
                    "posting_norm_reads": io["components"].get("pnorm", {}).get("reads", 0),
        ...
        out["identical_rows_and_scores"] = out["rows"]["global"] == out["rows"]["local"]
        out["identical_norm_lookup_count"] = out["summary"]["global"]["global_norm_lookups"] == out["summary"]["local"]["local_norm_lookups"]
        ...
        assert out["summary"]["local"]["global_norm_lookups"] == 0
