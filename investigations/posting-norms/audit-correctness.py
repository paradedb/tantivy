...
matrix = json.loads((root / (sys.argv[1] if len(sys.argv) > 1 else "audit-after-results.json")).read_text())
results = {}
with psycopg.connect("postgresql://mingying@127.0.0.1:29950/hn_benchmark?sslmode=disable", autocommit=True) as c:
    ...
    for name, measured in matrix.items():
        ...
        (root / "audit-correctness.json").write_text(json.dumps(results, indent=2))
        assert result["unscored_norm_lookups"] == 0
        c.execute("SELECT public.diagnostic_posting_norms(true)")
        assert c.execute(f"SELECT count(*) FROM hn_items WHERE {predicate}").fetchone()[0] == result["local"]["count"]
        ...
        print(name, result["local"], "unscored and count: zero norm reads", flush=True)
        (root / "audit-correctness.json").write_text(json.dumps(results, indent=2))
